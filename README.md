...
```
캐시 구성 요소는 부분 사전 렌더링(PPR)을 구현 하여 use cache두 가지 모두의 장점을 제공합니다.

#### 사용자가 경로를 방문할 때:

* 서버는 캐시된 콘텐츠를 포함하는 정적 셸을 보내 빠른 초기 로드를 보장합니다.
* 경계 로 둘러싸인 동적 섹션은 Suspense셸에서 대체 UI를 표시합니다.
* 폴백을 대체하기 위해 동적 부분만 렌더링되고, 준비되면 병렬로 스트리밍됩니다.
* 캐싱을 통해 초기 셸에 동적인 데이터를 포함할 수 있습니다.use cache
...
## 1교시
## 서버 및 클라이언트 구성 요소
* 기본적으로 레이아웃과 페이지는 서버 구성 요소 입니다.서버에서 데이터를 가져오고 UI의 일부를 렌더링할 수 있으며, 선택적으로 결과를 캐시하고 클라이언트로 스트리밍할 수 있습니다. 상호작용이나 브라우저 API가 필요한 경우 클라이언트 구성 요소를 사용할 수 있습니다.기능성을 더하기 위해.

* 이 페이지에서는 Next.js에서 서버 및 클라이언트 구성 요소가 작동하는 방식과 이를 사용하는 시기를 설명하고, 애플리케이션에서 이를 함께 구성하는 방법에 대한 예를 제공합니다.

* 서버와 클라이언트 구성요소를 언제 사용해야 하나요?
클라이언트 환경과 서버 환경은 서로 다른 기능을 가지고 있습니다. 서버 및 클라이언트 구성 요소를 사용하면 사용 사례에 따라 각 환경에서 로직을 실행할 수 있습니다.
...
* 첫 번째 콘텐츠 페인트(FCP) 개선그리고 콘텐츠를 점진적으로 클라이언트에게 스트리밍합니다.
#### 예를 들어, 해당 <Page>구성 요소는 게시물에 대한 데이터를 가져와서 <LikeButton>클라이언트 측 상호 작용을 처리하는 props로 전달하는 서버 구성 요소입니다.
...
* 서버에서 Next.js는 React API를 사용하여 렌더링을 조정합니다. 렌더링 작업은 개별 경로 세그먼트( 레이아웃 및 페이지 )별로 청크로 나뉩니다.

* 서버 구성 요소는 React Server Component Payload(RSC Payload)라는 특수한 데이터 형식으로 렌더링됩니다.
* 클라이언트 구성 요소 와 RSC 페이로드는 HTML을 미리 렌더링하는 데 사용됩니다 .
```
React Server Component Payload(RSC)란 무엇인가요?

RSC 페이로드는 렌더링된 React 서버 컴포넌트 트리의 압축된 바이너리 표현입니다. 클라이언트 측 React에서 브라우저의 DOM을 업데이트하는 데 사용됩니다. RSC 페이로드에는 다음이 포함됩니다.
...
1. HTML 은 사용자에게 경로의 빠르고 비대화형 미리보기를 즉시 보여주는 데 사용됩니다.
2. RSC 페이로드 는 클라이언트와 서버 구성 요소 트리를 조정하는 데 사용됩니다.
3. JavaScript 는 클라이언트 구성 요소를 수화하고 애플리케이션을 대화형으로 만드는 데 사용됩니다.
```
수분 공급이란 무엇인가?

수화는 이벤트 핸들러를 연결하기 위한 React의 프로세스입니다.DOM에 정적 HTML을 대화형으로 만듭니다.
```

### 이후의 항해
이후 탐색 시:

* RSC 페이로드는 즉각적인 탐색을 위해 미리 페치되어 캐시됩니다.
* 클라이언트 구성 요소는 서버에서 렌더링된 HTML 없이 클라이언트에서 전적으로 렌더링됩니다.
#### 예시
#### 클라이언트 구성 요소 사용
다음을 추가하여 클라이언트 구성 요소를 생성할 수 있습니다."use client"파일의 맨 위, 즉 가져온 파일 위에 지시문을 넣으세요.
...
```
"use client"서버와 클라이언트 모듈 그래프(트리) 사이의 경계를 선언하는 데 사용됩니다 .

파일에 로 표시되면 "use client"해당 파일의 모든 가져오기와 자식 구성 요소는 클라이언트 번들의 일부로 간주됩니다 . 즉, 클라이언트용으로 지정된 모든 구성 요소에 해당 지시문을 추가할 필요가 없습니다.

### JS 번들 크기 줄이기
클라이언트 JavaScript 번들의 크기를 줄이려면 'use client'UI의 큰 부분을 클라이언트 구성 요소로 표시하는 대신 특정 대화형 구성 요소에 추가하세요.

예를 들어, <Layout>구성 요소에는 로고와 탐색 링크와 같은 정적 요소가 대부분 포함되어 있지만 대화형 검색 창이 포함되어 있습니다. <Search />대화형이므로 클라이언트 구성 요소가 되어야 하지만 나머지 레이아웃은 서버 구성 요소로 유지될 수 있습니다.
...
```
또는 Hook을 사용하여 서버 구성 요소에서 클라이언트 구성 요소로 데이터를 스트리밍할 수 있습니다.use. 예를 참조하세요 .
```
알아두면 좋은 정보 : 클라이언트 구성 요소에 전달되는 Props는 직렬화 가능 해야 합니다.React로.
```
...
```
그런 다음 부모 서버 구성 요소(예: )에서 다음을 자식으로 <Page>전달할 수 있습니다 .<Cart><Modal>
...
```
이 패턴에서는 모든 서버 구성 요소(props 포함)가 서버에서 미리 렌더링됩니다. 생성된 RSC 페이로드에는 구성 요소 트리 내에서 클라이언트 구성 요소가 렌더링되어야 하는 위치에 대한 참조가 포함됩니다.

### 컨텍스트 제공자
React 컨텍스트현재 테마처럼 전역 상태를 공유하는 데 일반적으로 사용됩니다. 하지만 React 컨텍스트는 서버 컴포넌트에서 지원되지 않습니다.

컨텍스트를 사용하려면 다음을 허용하는 클라이언트 구성 요소를 만듭니다 children.
...
```
그런 다음 서버 구성 요소(예: layout)로 가져옵니다.
...
```
이제 서버 구성 요소는 공급자를 직접 렌더링할 수 있으며 앱 전체의 다른 모든 클라이언트 구성 요소는 이 컨텍스트를 사용할 수 있습니다.
```
알아두면 좋습니다 . 트리에서 공급자를 최대한 깊게 렌더링해야 합니다 . 전체 문서가 아닌 ThemeProvider래핑만 처리하는 방식에 유의하세요 . 이렇게 하면 Next.js가 서버 구성 요소의 정적 부분을 더 쉽게 최적화할 수 있습니다.{children}<html>
```
### 타사 구성 요소
...
```
하지만 서버 컴포넌트 내에서 직접 사용하려고 하면 오류가 발생합니다. 이는 Next.js가 <Carousel />클라이언트 전용 기능을 사용하고 있다는 것을 알지 못하기 때문입니다.

이 문제를 해결하려면 클라이언트 전용 기능에 의존하는 타사 구성 요소를 자체 클라이언트 구성 요소로 래핑할 수 있습니다.
...
```
이제 <Carousel />서버 구성 요소 내에서 직접 사용할 수 있습니다.
...
```
### 환경 오염 예방
JavaScript 모듈은 서버 및 클라이언트 구성 요소 모듈 간에 공유될 수 있습니다. 즉, 실수로 서버 전용 코드를 클라이언트로 가져올 수 있습니다. 예를 들어 다음 함수를 살펴보겠습니다.
...
그런 다음, 서버 전용 코드가 포함된 파일에 패키지를 가져옵니다.
...
```
이제 모듈을 클라이언트 구성 요소로 가져오려고 하면 빌드 타임 오류가 발생합니다.

해당 client-only패키지객체 에 액세스하는 코드와 같이 클라이언트 전용 논리를 포함하는 모듈을 표시하는 데 사용할 수 있습니다 window.

Next.js에서 server-only또는 를 설치하는 것은 선택 사항client-only 입니다 . 하지만 린팅 규칙에서 불필요한 종속성을 표시하는 경우, 문제를 방지하기 위해 해당 종속성을 설치할 수 있습니다.

```tsx
pnpm 엔피엠 방사 혈액 요소 질소 
pnpm add server-only
```
Next.js는 모듈이 잘못된 환경에서 사용될 때 더 명확한 오류 메시지를 제공하기 위해 내부적으로 처리하고 server-only가져 client-only옵니다. NPM의 이러한 패키지 내용은 Next.js에서 사용되지 않습니다.

Next.js는 또한 TypeScript 구성에 대한 server-only자체 유형 선언을 제공합니다 .client-onlynoUncheckedSideEffectImports활성화되어 있습니다.
